...
@section('title', 'Kategori Makanan & Minuman - PaDi UMKM')
        ...
        <div class="grid grid-cols-1 gap-6 items-start md:grid-cols-12">
            <aside class="space-y-4 md:col-span-3">
                <div class="rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                    ...
                    @php
                        $filters = [
                            ...
                            ['title' => 'Rentang Harga', 'options' => ['< Rp500 ribu', 'Rp500 rb - Rp1 jt', 'Rp1 jt - Rp5 jt', '> Rp5 jt']],
                        ...
                        @foreach ($filters as $filter)
                            <details class="group rounded-xl border border-slate-100 bg-slate-50/60" {{ $loop->first ? 'open' : '' }}>
                                <summary class="flex cursor-pointer items-center justify-between px-4 py-3 text-sm font-semibold text-slate-800">
                                    {{ $filter['title'] }}
                                    <svg class="h-4 w-4 text-slate-400 transition group-open:rotate-180" fill="none" stroke="currentColor" stroke-width="2"
                                        viewBox="0 0 24 24">
                                        <path d="m6 9 6 6 6-6" />
                                    </svg>
                                </summary>
                ...
                <div class="rounded-xl bg-slate-900 p-5 text-white shadow-sm">
                    ...
                    <p class="mt-2 text-xs text-white/80">Tim procurement PaDi siap bantu kurasi vendor terbaik sesuai kebutuhan instansi Anda.</p>
                    <button class="mt-4 w-full rounded-xl bg-white/15 py-2 text-sm font-semibold text-white transition hover:bg-white/30">Hubungi Konsultan</button>
                </div>
            </aside>
            ...
            <div class="space-y-4 md:col-span-9">
                ...
                <p class="text-sm text-slate-600">Menampilkan 1 - 50 produk dari total 121,277 <span class="font-semibold text-slate-900">Makanan dan Minuman</span></p>

                @php
                    $products = collect(range(1, 24))->map(function ($i) {
                        return [
                            'title' => 'Kurasi paket kudapan #' . $i,
                            'vendor' => 'UMKM Kuliner Nusantara ' . $i,
                            'price' => 'Rp' . number_format(250000 + $i * 15000, 0, ',', '.'),
                            'location' => $i % 2 === 0 ? 'Bandung' : 'Jakarta',
                            'status' => $i % 3 === 0 ? 'Promo' : 'Ready',
                            'image' => 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&w=600&q=80&sig=' . $i,
                        ];
                    });
                @endphp
                    ...
                    @foreach ($products as $product)
                        <article class="flex h-full flex-col overflow-hidden rounded-md border border-slate-100 bg-white shadow-sm">
                            <div class="relative">
                                ...
                                <span class="absolute left-2 top-2 rounded-full bg-sky-600 px-2.5 py-0.5 text-[11px] font-semibold text-white">
                                    UMKM
                                </span>
                            </div>
                            <div class="flex flex-1 flex-col gap-2 p-3">
                                <span class="inline-flex w-fit items-center gap-1 rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[11px] font-semibold text-slate-700">
                ...
                <div class="flex flex-wrap items-center gap-3 rounded-lg border border-slate-200 bg-white px-5 py-4 text-sm text-slate-600">
                    ...
                    <div class="ml-auto flex items-center gap-2">
                        ...
                        <div class="flex items-center gap-1 text-xs">
                            @foreach ([1, 2, 3, 4, 5] as $page)
                                <button class="h-7 w-7 rounded-full {{ $page === 1 ? 'bg-sky-500 text-white' : 'bg-slate-100 text-slate-600' }}">
                                    {{ $page }}
                                </button>
                            @endforeach
                        </div>
                ...
                <article class="space-y-4 rounded-xl bg-white p-6 shadow">
                    <h2 class="text-2xl font-bold text-slate-900">Jual Ragam Pengadaan Makanan dan Minuman Hanya di PaDi UMKM</h2>
                    <p class="text-sm text-slate-600">Solusi kurasi bagi instansi yang ingin kolaborasi dengan pelaku UMKM, kategori makanan dan minuman di PaDi
                        menghadirkan pilihan hampers, paket konsumsi, hingga produk kesehatan yang siap dikirimkan ke seluruh Indonesia.</p>
                    <div class="space-y-4 text-sm text-slate-600">
                        <div>
                            <h3 class="text-base font-semibold text-slate-900">Sambut Harimu dengan Produk Bergizi</h3>
                            <p>Penuhi kebutuhan pantry kantor atau acara perusahaan dengan kudapan sehat, kopi botolan, hingga frozen food dari vendor
                        ...
                        <div>
                            ...
                            <p>Paket parsel dan hampers eksklusif siap dikustomisasi sesuai tema perusahaan. Tim vendor UMKM tersebar di berbagai kota,
                        ...
                        <div>
                            ...
                            <p>Jika Anda pelaku UMKM kategori makanan dan minuman, segera daftar sebagai mitra PaDi UMKM untuk menikmati akses tender dan
