...
const form = useForm({
    ...
    is_pkp: props.product?.is_pkp ?? false,
    is_tkdn: props.product?.is_tkdn ?? false,
    images: [] as File[],
    ...
    <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative flex items-center gap-2">
                        <Input id="slug" :model-value="form.slug" :disabled="isSlugLocked"
                            :class="[form.errors.slug ? 'border-red-500' : '', isSlugLocked ? 'bg-slate-50' : '']"
                            class="pr-28" required @update:model-value="handleSlugInput" />
                ...
                <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                ...
                                <Command>
                                    ...
                                    <CommandGroup>
                                        <CommandItem v-for="option in filteredCategoryOptions" :key="option.id"
                                            :value="option.name" @select="() => selectCategory(option)">
                    ...
                    <div class="space-y-2">
                        ...
                        <Select v-model="form.item_type">
                            <SelectTrigger id="item_type" class="w-full"
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="space-y-2">
                        ...
                        <Select v-model="form.visibility_scope">
                            ...
                            <SelectContent>
                                <SelectItem v-for="option in visibilityOptions" :key="option.value"
                                    :value="option.value">
                                    {{ option.label }}
                                </SelectItem>
                            </SelectContent>
            ...
            <CardContent class="space-y-4">
                <div class="space-y-2">
                    ...
                    <div class="flex flex-col gap-1">
                        <Button type="button" variant="outline" size="sm" @click="triggerImagePicker"
                    ...
                    <div v-if="imagePreviews.length" class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                        ...
                        <div v-for="(preview, index) in imagePreviews" :key="preview.id"
                            ...
                            <button type="button"
                                ...
                                aria-label="Hapus gambar" :disabled="form.processing"
            ...
            <CardContent class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <Input id="price" v-model="form.price" type="number" min="0" placeholder="0" required
                            class="pl-10" :class="form.errors.price ? 'border-red-500' : ''" />
                    </div>
                ...
                <div class="space-y-2">
                    <div class="flex items-center justify-between">
                        ...
                        <Badge v-if="discountPercentage" variant="destructive"
                            class="text-[11px] rounded-full px-2.5 py-0.5">
                            -{{ discountPercentage }}%
                        </Badge>
                    </div>
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-slate-500">Rp</span>
                        <Input id="sale_price" v-model="form.sale_price" type="number" min="0" placeholder="0"
                ...
                <div class="space-y-4">
                    <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <Button v-if="showEditLocationButton" type="button" size="sm" variant="ghost"
                            class="text-blue-600 hover:text-blue-700" :disabled="form.processing"
                            @click="enableLocationEditing">
                            Ubah lokasi produk
                        </Button>
                    </div>

                    <RegionSelector v-model:provinceId="form.location_province_id"
                        v-model:cityId="form.location_city_id" v-model:districtId="form.location_district_id"
                        :show-district="true" :disabled="locationFieldsDisabled" :errors="{
                    ...
                    <div class="space-y-2">
                        ...
                        <Input id="location_postal_code" v-model="form.location_postal_code" placeholder="Kode pos asal"
                            required :disabled="locationFieldsDisabled"
                            :class="form.errors.location_postal_code ? 'border-red-500' : ''" />
        ...
        <Card>
            ...
            <CardContent class="grid gap-4 md:grid-cols-3">
                <label
                    class="flex items-center justify-between rounded-md border border-slate-200 bg-slate-50/60 px-4 py-3">
                    <div>
                ...
                <label
                    class="flex items-center justify-between rounded-md border border-slate-200 bg-slate-50/60 px-4 py-3">
                    <div>
                        <p class="text-sm font-medium text-slate-800">PKP</p>
                        <p class="text-xs text-slate-500">Pemungut Pajak Pertambahan Nilai.</p>
                    </div>
                ...
                <label
                    class="flex items-center justify-between rounded-md border border-slate-200 bg-slate-50/60 px-4 py-3">
                    <div>
                        <p class="text-sm font-medium text-slate-800">TKDN</p>
                        <p class="text-xs text-slate-500">Tingkat komponen dalam negeri.</p>
                    </div>
