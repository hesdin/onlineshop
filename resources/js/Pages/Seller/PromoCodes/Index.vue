...
const debouncedSearch = useDebounceFn((value: string) => {
  router.get('/seller/promo-codes', { search: value || undefined }, {
    preserveState: true,
    replace: true,
    preserveScroll: true,
  });
...
const deletePromoCode = () => {
  ...
  router.delete(`/seller/promo-codes/${deletingPromoCode.value.id}`, {
    preserveScroll: true,
    onFinish: () => {
      ...
      h('div', { class: 'flex justify-end gap-3' }, [
        h(
          'span',
          {
            ...
            onClick: () => router.visit(`/seller/promo-codes/${row.original.id}/edit`),
          },
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-2xl font-semibold text-slate-900">Promo Saya</h1>
        <p class="text-sm text-slate-500">Kelola kode promo penarik minat pelanggan toko Anda.</p>
      </div>
      <Button as-child>
        <Link href="/seller/promo-codes/create">
          <Plus class="h-4 w-4" />
          Tambah Promo
        </Link>
      </Button>
