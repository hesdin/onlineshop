...
const form = useForm({
  ...
  starts_at: props.promoCode.starts_at ? new Date(props.promoCode.starts_at).toISOString().slice(0, 16) : '',
  ends_at: props.promoCode.ends_at ? new Date(props.promoCode.ends_at).toISOString().slice(0, 16) : '',
  quota: props.promoCode.quota,
  is_active: props.promoCode.is_active,
});

const submit = () => {
  form.put(`/seller/promo-codes/${props.promoCode.id}`, {
    preserveScroll: true,
  });
};
</script>
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between gap-3">
      <div class="space-y-1">
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">Edit Promo</h1>
        <p class="text-sm text-slate-500">Perbarui informasi promo <strong>{{ promoCode.code }}</strong> Anda.</p>
      </div>
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>Sesuaikan detail informasi promo Anda di bawah ini.</CardDescription>
      </CardHeader>
      <CardContent>
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="space-y-2">
            ...
            <Textarea id="description" v-model="form.description" rows="2"
              placeholder="Jelaskan syarat dan ketentuan promo ini..." />
          </div>
