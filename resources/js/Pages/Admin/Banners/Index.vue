<script setup lang="ts">
import AdminDashboardLayout from '@/Layouts/AdminDashboardLayout.vue';
import { Head, Link, router, usePage } from '@inertiajs/vue3';
import { computed, h, ref, watch } from 'vue';
import { useDebounceFn } from '@vueuse/core';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent,
  AlertDialogDescription,
  AlertDialogFooter,
  AlertDialogHeader,
  AlertDialogTitle,
} from '@/components/ui/alert-dialog';
import { DataTable } from '@/components/ui/data-table';
import { Trash2, Edit2, Plus, CheckCircle2, Search } from 'lucide-vue-next';
import type { ColumnDef } from '@tanstack/vue-table';
import AlertBanner from '@/components/AlertBanner.vue';

type BannerRow = {
  id: number;
  title: string;
  type: string;
  type_label: string;
  position: number;
  is_active: boolean;
  image_url: string | null;
  created_at: string;
};

const props = defineProps<{
  banners: {
    data: BannerRow[];
    total: number;
    links: { label: string; url: string | null; active: boolean }[];
    prev_page_url: string | null;
    next_page_url: string | null;
  };
  filters: {
    search?: string | null;
  };
}>();

defineOptions({
  layout: AdminDashboardLayout,
});

const page = usePage();
const flash = computed(() => (page.props.flash ?? {}) as Record<string, string>);
const flashSuccess = computed(() => flash.value.success ?? '');
const showSuccess = ref(false);
const search = ref(props.filters.search ?? '');
const deleteDialogOpen = ref(false);
const deletingBanner = ref<BannerRow | null>(null);

// Watch flash object directly to detect new messages even if content is the same
watch(() => page.props.flash, (newFlash) => {
  const flashData = newFlash as Record<string, string> | undefined;
  if (flashData?.success) {
    showSuccess.value = true;
    setTimeout(() => {
      showSuccess.value = false;
    }, 5000);
  }
}, { deep: true, immediate: true });

const debouncedSearch = useDebounceFn((value: string) => {
  router.get('/admin/banners', { search: value || undefined }, {
    preserveState: true,
    replace: true,
    preserveScroll: true,
  });
}, 400);

watch(search, (value) => {
  debouncedSearch(value);
});

const requestDelete = (banner: BannerRow) => {
  deletingBanner.value = banner;
  deleteDialogOpen.value = true;
};

const deleteBanner = () => {
  if (!deletingBanner.value) return;
  router.delete(`/admin/banners/${deletingBanner.value.id}`, {
    preserveScroll: true,
    onFinish: () => {
      deleteDialogOpen.value = false;
      deletingBanner.value = null;
    },
  });
};

const numberedPaginationLinks = computed(() =>
  (props.banners.links ?? []).filter((link) => Number.isInteger(Number(link.label))),
);

const paginateTo = (url?: string | null) => {
  if (!url) return;
  router.visit(url, {
    preserveScroll: true,
    preserveState: true,
    replace: true,
  });
};

const columns = computed<ColumnDef<BannerRow>[]>(() => [
  {
    accessorKey: 'image_url',
    header: () => 'Gambar',
    cell: ({ row }) =>
      h('div', { class: 'h-12 w-20 overflow-hidden rounded bg-slate-100' }, [
        row.original.image_url
          ? h('img', {
            src: row.original.image_url,
            alt: row.original.title,
            class: 'h-full w-full object-cover',
          })
          : h('span', { class: 'flex h-full items-center justify-center text-xs text-slate-400' }, 'No image'),
      ]),
  },
  {
    accessorKey: 'title',
    header: () => 'Judul',
    cell: ({ row }) =>
      h('span', { class: 'font-semibold text-slate-900' }, row.original.title),
  },
  {
    accessorKey: 'type_label',
    header: () => 'Tipe',
    cell: ({ row }) =>
      h(
        'span',
        {
          class: row.original.type === 'hero_slider'
            ? 'rounded-sm bg-sky-100 px-2 py-1 text-xs font-semibold text-sky-700'
            : 'rounded-sm bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-700',
        },
        row.original.type_label,
      ),
  },
  {
    accessorKey: 'position',
    header: () => 'Posisi',
    cell: ({ row }) =>
      h('span', { class: 'text-sm text-slate-600' }, String(row.original.position)),
  },
  {
    accessorKey: 'is_active',
    header: () => 'Status',
    cell: ({ row }) =>
      h(
        'span',
        {
          class: row.original.is_active
            ? 'rounded-sm bg-green-100 px-2 py-1 text-xs font-semibold text-green-700'
            : 'rounded-sm bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-600',
        },
        row.original.is_active ? 'Aktif' : 'Tidak Aktif',
      ),
  },
  {
    id: 'actions',
    header: () => 'Aksi',
    cell: ({ row }) =>
      h('div', { class: 'flex justify-end gap-3' }, [
        h(
          'span',
          {
            class:
              'inline-flex h-8 w-8 items-center justify-center rounded-full ' +
              'cursor-pointer text-slate-500 hover:bg-slate-100 hover:text-slate-900',
            onClick: () => router.visit(`/admin/banners/${row.original.id}/edit`),
          },
          [h(Edit2, { class: 'h-[15px] w-[15px]' })],
        ),
        h(
          'span',
          {
            class:
              'inline-flex h-8 w-8 items-center justify-center rounded-full ' +
              'cursor-pointer text-slate-500 hover:bg-red-50 hover:text-red-600',
            onClick: () => requestDelete(row.original),
          },
          [h(Trash2, { class: 'h-[15px] w-[15px]' })],
        ),
      ]),
    meta: { class: 'text-right w-32' },
  },
]);
</script>

<template>
  <div class="space-y-6">

    <Head title="Banner" />

    <!-- Floating Success Alert -->
    <Teleport to="body">
      <div v-if="showSuccess && flashSuccess"
        class="fixed top-20 left-1/2 -translate-x-1/2 z-[9999] min-w-[600px] max-w-2xl shadow-lg rounded-lg overflow-hidden">
        <AlertBanner type="success" :message="flashSuccess" :show="showSuccess" :dismissible="true"
          @close="showSuccess = false" />
      </div>
    </Teleport>

    <div class="flex flex-wrap items-center justify-between gap-3">
      <h1 class="text-2xl font-semibold text-slate-900">Manajemen Banner</h1>
      <Button as-child>
        <Link href="/admin/banners/create">
          <Plus class="h-4 w-4" />
          Tambah Banner
        </Link>
      </Button>
    </div>

    <section class="space-y-4">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="relative w-full md:w-64">
          <span class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400">
            <Search class="h-4 w-4" />
          </span>
          <Input v-model="search" placeholder="Cari banner" class="w-full pl-9" />
        </div>
      </div>

      <div class="overflow-hidden rounded-sm border border-slate-200 bg-white">
        <DataTable :columns="columns" :data="banners.data" />
      </div>

      <div class="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500">
        <p>Menampilkan {{ banners.data.length }} dari {{ banners.total }} banner.</p>
        <div class="flex flex-wrap items-center gap-1">
          <Button variant="outline" size="sm" :disabled="!banners.prev_page_url"
            @click="paginateTo(banners.prev_page_url)">
            Sebelumnya
          </Button>
          <Button v-for="link in numberedPaginationLinks" :key="link.label" size="sm"
            :variant="link.active ? 'default' : 'outline'" :disabled="!link.url" @click="paginateTo(link.url)">
            {{ link.label }}
          </Button>
          <Button variant="outline" size="sm" :disabled="!banners.next_page_url"
            @click="paginateTo(banners.next_page_url)">
            Selanjutnya
          </Button>
        </div>
      </div>
    </section>

    <AlertDialog :open="deleteDialogOpen" @update:open="(value) => (deleteDialogOpen = value)">
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Hapus banner?</AlertDialogTitle>
          <AlertDialogDescription>
            Banner <strong>{{ deletingBanner?.title }}</strong> akan dihapus permanen.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel @click="deleteDialogOpen = false">Batal</AlertDialogCancel>
          <AlertDialogAction class="bg-red-600 hover:bg-red-700" @click="deleteBanner">
            Hapus
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  </div>
</template>
